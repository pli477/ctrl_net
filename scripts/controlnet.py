...
class Script(scripts.Script):
    ...
    def uigroup(self, tabname, is_img2img, elem_id_tabname):
        ...
        with gr.Tabs(visible=False):
            with gr.Tab(label='Single Image') as upload_tab:
                with gr.Row(visible=False).style(equal_height=True):
        ...
        with gr.Row():
            ...
            trigger_preprocessor = ToolButton(value=trigger_symbol, visible=False, elem_id=f'{elem_id_tabname}_{tabname}_controlnet_trigger_preprocessor')
            ...
            refresh_models = ToolButton(visible=False, value=refresh_symbol, elem_id=f'{elem_id_tabname}_{tabname}_controlnet_refresh_models')
        ...
        def build_sliders(module, pp):
            ...
            if module not in preprocessor_sliders_config:
                ...
                while len(grs) < 3:
                    grs.append(gr.update(visible=False, interactive=False))
                grs.append(gr.update(visible=False))
        ...
        control_mode = gr.Radio(visible=False, choices=[e.value for e in external_code.ControlMode], value=default_unit.control_mode.value, label="Control Mode", elem_id=f'{elem_id_tabname}_{tabname}_controlnet_control_mod_radio')

        resize_mode = gr.Radio(visible=False,choices=[e.value for e in external_code.ResizeMode], value=default_unit.resize_mode.value, label="Resize Mode", elem_id=f'{elem_id_tabname}_{tabname}_controlnet_resize_mode_radio')

        loopback = gr.Checkbox(visible=False, label='[Loopback] Automatically send generated images to this ControlNet unit', value=default_unit.loopback, elem_id=f'{elem_id_tabname}_{tabname}_controlnet_automatically_send_generated_images_checkbox')
    ...
    def parse_remote_call(self, p, unit: external_code.ControlNetUnit, idx):
        ...
        # unit.module = "openpose_full"
        unit.model = selector(p, "control_net_model", unit.model, idx)
        # unit.model = list(global_state.cn_models.keys())[-1]
